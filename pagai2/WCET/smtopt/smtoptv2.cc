...
bool readProblem(int argc, char *argv[], string &smtFormula, char * &varMaximize, string &cutsfilename){
	string line;
...
bool update_cuts(const Z3_context &ctx, std::string cutsfilename){
    ...
    // counting the number of cut variables... (just for display)
	for (unsigned i = 0; i < num_decl; i++) {
		Z3_symbol symb = Z3_get_decl_name(ctx, Z3_get_smtlib_decl(ctx, i));
		Z3_string s = Z3_get_symbol_string(ctx,  symb);
		std::string name(s);
		std::size_t found = name.find("cut");
    ...
    string current_cut;
    int current_cut_maxbound;
    string line;
    ifstream cutsfile (cutsfilename.c_str());
    if (cutsfile.is_open()) {
        while ( getline (cutsfile,line) ) {
			istringstream iss(line);
			iss >> current_cut;
			iss >> current_cut_maxbound;
            cout << current_cut << '\n';
	        for (unsigned i = 0; i < num_decl; i++) {
	        	Z3_symbol symb = Z3_get_decl_name(ctx, Z3_get_smtlib_decl(ctx, i));
	        	Z3_string s = Z3_get_symbol_string(ctx,  symb);
	        	std::string name(s);
	        	if (name.compare(current_cut) == 0) {
	        		treated++;
	        		std::cout << "cut " << treated << "/" << nb_cuts << "\n";
	        		decl = Z3_get_smtlib_decl(ctx, i);
	        		var = Z3_mk_const(ctx, symb, Z3_mk_int_sort(ctx));
